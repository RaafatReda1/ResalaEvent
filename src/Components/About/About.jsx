...
const About = () => {
  ...
  const texts = [
    `إيه هو Event ما وراء الطب؟`,
    `
مش مجرد Event.
دي رحلة لاكتشاف اللي ورا البالطو.`,

    `تجارب، حكايات، وإلهام من دكاترة كبار عاشوا الرحلة قبلك.`,
    `مين هما أطباء الخير؟`,
    `
طلبة طب اختاروا إن علمهم يبقى له أثر.`,

    `أطباء الخير
فريق بيجمع بين الطب والتطوع.`,

    `هدفنا إيه من Event ما وراء الطب؟`,
    `
نوريك إن الطب أكبر من كتاب ومحاضرة.`,

    `نساعدك تشوف الطريق أوضح.
وتعرف إيه اللي مستنيك ورا البالطو.`,

    `مين هي جمعية رسالة؟`,

    `
جمعية رسالة… رسالة محبة قبل ما تكون رسالة عطاء. 
`,

    `هدفها نشر المحبة.
ونشر روح التعاون والمساعدة.`,

    `مش مجرد كلام.
لكن قيم تتحول لعمل حقيقي.`,

    `والهدف؟
نترك أثرًا حقيقيًا في المجتمع.`,
  ];
  ...
  return (
    <div className={styles.aboutContainer} id="about" ref={aboutSectionRef}>
      <MagicRings />
